...
float sdf(vec3 p) {
    ...
    float final = sphereSDF(p1 - vec3(-0.5, 0.0, 0.0), 0.28);
    float nextSphere = sphereSDF(p2 - vec3(0.55, 0.0, 0.0), 0.24);
    final = smin(final, nextSphere, 0.1);
    nextSphere = sphereSDF(p2 - vec3(-0.8, 0.0, 0.0), 0.16);
    final = smin(final, nextSphere, 0.1);
    nextSphere = sphereSDF(p3 - vec3(1.0, 0.0, 0.0), 0.12);
    final = smin(final, nextSphere, 0.1);
    nextSphere = sphereSDF(p4 - vec3(0.45, -0.45, 0.0), 0.12);
    final = smin(final, nextSphere, 0.1);
...
void main() {
    ...
    if (t > 0.0) {
        ...
        float lighting = 0.5 + 0.3 * dot(normal, normalize(vec3(0.2, 0.7, 1.0)));
        float edge = smoothstep(0.0, 0.1, sdf(p));
        vec3 dark = vec3(0.08, 0.08, 0.08);
        vec3 mid = vec3(0.35, 0.35, 0.35);
        vec3 light = vec3(0.8, 0.8, 0.8);
        float shadow = 0.5 * (1.0 - dot(normal, normalize(vec3(0.0, 0.0, 1.0))));
        vec3 shadowColor = mix(mid, dark, shadow * edge);
        float rim = pow(1.0 - max(dot(normal, -ray), 0.0), 2.5);
        vec3 highlight = vec3(1.0, 1.0, 0.95) * rim * 0.25;
        color = mix(shadowColor, light, lighting) + highlight;
